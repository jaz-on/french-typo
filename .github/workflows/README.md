...
The Release Drafter workflow automatically generates release notes from merged pull requests, using a format aligned with [Keep a Changelog](https://keepachangelog.com/) standard.
...
3. Groups changes by labels using Keep a Changelog format:
   ...
   - `maintenance` / `chore` / `refactor` / `code-quality` → **### Code Quality**
   - `documentation` / `docs` → **### Documentation**
4. Includes contributors list automatically
5. Generates a "Full Changelog" link comparing tags

### Format alignment

The release notes generated by Release Drafter use the same format as `CHANGELOG.md` (Keep a Changelog), ensuring consistency between:
- GitHub release notes (generated by Release Drafter)
- `CHANGELOG.md` (manually maintained)
- WordPress.org `readme.txt` changelog

### Usage

1. Label your pull requests with appropriate labels (see list above)
...
**Note:** For releases without PRs (like initial releases with commits directly on `main`), Release Drafter won't generate notes. The Create Release ZIP workflow will automatically use `CHANGELOG.md` (Keep a Changelog format) as a fallback, ensuring consistent formatting.
...
7. **Generate release notes:** Automatically generates comprehensive release notes with the following priority:
   - **If Release Drafter notes exist:** Uses the automatically generated notes from merged PRs (format aligned with Keep a Changelog)
   - **CHANGELOG.md (preferred fallback):** Parses the Keep a Changelog section for the targeted version
   ...
   **Note:** Since Release Drafter now uses the Keep a Changelog format, the notes will be consistent whether generated from PRs or from `CHANGELOG.md`.
